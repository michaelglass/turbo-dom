// Parse-throughput benchmark: turbodom vs parse5 vs happy-dom vs jsdom.
...
const engines = [
  { name: 'turbodom parseBuffer()', fn: (h) => parseBuffer(h) },
  { name: 'turbodom parse()', fn: (h) => parse(h) },
  { name: 'turbodom parseRaw()', fn: (h) => parseRaw(h) },
...
for (const fx of fixtures) {
  ...
  const baseline = results.find((r) => r.name === 'turbodom parseBuffer()');
...
console.log('rel = speed relative to turbodom parseBuffer() — the SoA fast path (>1 faster, <1 slower).');
