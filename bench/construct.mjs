...
// construction + a typical light test, fast-dom (lazy) vs jsdom vs happy-dom.
// Plus the lazy-vs-eager split within fast-dom and a surface-usage histogram.
...
row('fast-dom', f); row('jsdom', j); row('happy-dom', h);
console.log(`\nfast-dom is ${(f / j).toFixed(1)}x jsdom and ${(f / h).toFixed(1)}x happy-dom on the light-test path.`);

// Lazy vs eager WITHIN fast-dom: same construction, touch little vs touch everything.
console.log('\nLazy payoff inside fast-dom (ops/sec)\n');
