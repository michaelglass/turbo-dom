...
test('conformance ordering holds: turbodom >= jsdom >> happy-dom', () => {
  ...
  // turbodom (html5ever) must be at least as conformant as jsdom
  assert.ok(
    fast.rate >= jsd.rate,
    `turbodom ${fast.rate.toFixed(2)}% < jsdom ${jsd.rate.toFixed(2)}%`
  );
  ...
  assert.ok(fast.rate >= 98.0, `turbodom regressed below 98%: ${fast.rate.toFixed(2)}%`);
});
